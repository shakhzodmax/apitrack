...
class ApiCheckEveryMonth extends Command
{
    protected $signature = 'app:api-check-every-month';
    protected $description = 'Har oylik tekshiruv';

    public function handle()
    {
        $getApis = DB::connection('oracle')->table('tbldata')->where('type_id', 1200)->where('int03', 30)->where('astate', 1)->get();
        ...
        foreach ($getApis as $getApi) {
            ...
            if ($checkStatus) {
                ...
                $this->info('Har oylik tekshiruv xabari telegramga jo\'natildi.');
            }
        }
    }
}
